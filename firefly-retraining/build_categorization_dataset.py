...
Reads the last 7 days of production-logs/categorization/ events, merges them
50/50 with the external CE Survey training data, and writes a versioned
dataset to retraining-data/categorization/v=YYYY-MM-DD/.

Triggered by cron every Sunday at 3 AM (see cron/firefly-retraining.cron).
Follows the 6-step pipeline from the retraining_data_pipeline_flow diagram:
...
import config
import utils
from schemas import CategorizationEvent
...
def step_1_ingest(con, cutoff_iso: str):
    """Read JSONL from production-logs + Parquet from training-data."""
    log.info("STEP 1 — Ingest")

    prod_glob = (
        f"s3://{config.BUCKET_PRODUCTION_LOGS}/{config.PREFIX_CATEGORIZATION_LOGS}/"
        f"year=*/month=*/day=*/hour=*/*.jsonl"
    )
    try:
        ...
        """).df()
    except duckdb_error_types() as e:
        log.warning("No production events found (%s) — first run is OK", e)
        prod_df = pd.DataFrame()

    ext_glob = f"s3://{config.BUCKET_TRAINING_DATA}/{config.PREFIX_CE_CATEGORIZATION}/*.parquet"
...
def step_2_validate(prod_df: pd.DataFrame) -> pd.DataFrame:
    """Pydantic validation + dedup by event_id."""
    ...
    if reject_rate > config.MAX_SCHEMA_REJECT_RATE:
        fail(f"Schema reject rate {reject_rate:.1%} exceeds limit {config.MAX_SCHEMA_REJECT_RATE:.0%}")
...
def step_3_transform(prod_df: pd.DataFrame, ext_df: pd.DataFrame):
    """Derive feedback_weight from user_action; tag data source."""
    ...
    # Give external rows a deterministic early timestamp so temporal split
    # places them in train rather than test.
...
def step_4_weight_mix(prod_df: pd.DataFrame, ext_df: pd.DataFrame) -> pd.DataFrame:
    ...
    mixed = pd.concat([prod_sample, ext_sample], ignore_index=True)
    ...
    # Quality gates on the full mixed set
    ...
    if len(cat_counts) < config.MIN_UNIQUE_CATEGORIES:
        fail(f"Only {len(cat_counts)} categories — need ≥{config.MIN_UNIQUE_CATEGORIES}")
    sparse = cat_counts[cat_counts < config.MIN_EXAMPLES_PER_CATEGORY]
    if len(sparse) > 0:
        log.warning("  %d categories have <%d examples: %s",
                    len(sparse), config.MIN_EXAMPLES_PER_CATEGORY, sparse.index.tolist()[:5])
...
def duckdb_error_types():
    import duckdb as _d
    return (_d.IOException, _d.CatalogException, _d.BinderException, Exception)
...
def main():
    ...
    log.info("=" * 70)
    log.info("Categorization retraining build — version=%s", utils.version_folder(now))
    log.info("Lookback: last %d days (cutoff %s)", config.CATEGORIZATION_LOOKBACK_DAYS, cutoff)
    log.info("=" * 70)
